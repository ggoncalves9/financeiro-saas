                ...
                <div class="card-body">
                    <div class="d-flex justify-content-between">
                        <div>
                            ...
                            <h4>{{ $stats['total_balance_formatted'] }}</h4>
                        </div>
    ...
    @if($accounts->count() > 0)
        <div class="row">
            ...
            <div class="col-lg-6 col-xl-4 mb-4">
                <div class="card account-card h-100 {{ $account->is_active ? 'border-success' : 'border-secondary' }}">
                    <div class="card-header d-flex justify-content-between align-items-center">
                        ...
                        <div class="dropdown">
                            ...
                            <ul class="dropdown-menu">
                                ...
                                <li><a class="dropdown-item" href="#" onclick="updateBalance({{ $account->id }})">
                                    <i class="fas fa-sync me-2"></i>Atualizar Saldo
                                </a></li>
                                @if($account->is_active)
                                    <li><a class="dropdown-item text-warning" href="#" onclick="deactivateAccount({{ $account->id }})">
                                        <i class="fas fa-pause me-2"></i>Desativar
                                    </a></li>
                                @else
                                    <li><a class="dropdown-item text-success" href="#" onclick="activateAccount({{ $account->id }})">
                    ...
                    <div class="card-body">
                        ...
                        <div class="text-center mb-3">
                            ...
                            <h3 class="mb-0 {{ $account->balance >= 0 ? 'text-success' : 'text-danger' }}">
                                {{ $account->formatted_balance }}
                            </h3>
                            <small class="text-muted">
                                Atualizado em {{ $account->updated_at->format('d/m/Y H:i') }}
                            </small>
                        </div>
                        ...
                        @if($account->account_number)
                            <div class="mt-3 text-center">
                                <small class="text-muted">
                                    Conta: {{ $account->masked_account_number }}
                                </small>
                            </div>
                        @endif

                        <!-- Recent Transactions Preview -->
                        @if($account->recentTransactions && $account->recentTransactions->count() > 0)
                            <hr>
                            <div class="small">
                                <div class="text-muted mb-2 fw-bold">Últimas Movimentações</div>
                                @foreach($account->recentTransactions->take(3) as $transaction)
                                    <div class="d-flex justify-content-between mb-1">
                                        <span>{{ Str::limit($transaction->description, 20) }}</span>
                        ...
                        @if($account->is_active)
                            ...
                            <div class="d-grid gap-2">
                                <div class="btn-group" role="group">
                                    <button class="btn btn-sm btn-outline-success" onclick="addTransaction({{ $account->id }}, 'deposit')">
                                        <i class="fas fa-plus me-1"></i>Depósito
                                    </button>
                                    <button class="btn btn-sm btn-outline-danger" onclick="addTransaction({{ $account->id }}, 'withdrawal')">
